...
WHERE
    EXISTS( SELECT 
        ...
        FROM
            (SELECT 
                hd.tong_tien, lk.id_loai_khach
            FROM
                hop_dong AS hd
            JOIN khach_hang ON khach_hang.id_khach_hang = hd.id_khach_hang
            JOIN loai_khach AS lk ON lk.id_loai_khach = khach_hang.id_loai_khach
            WHERE
                hd.tong_tien > 10000000
                    AND lk.id_loai_khach = 2) AS c);
...
WHERE
    ...
    FROM
        (SELECT 
            kh.id_khach_hang, hd.id_hop_dong, hd.ngay_lam_hop_dong
        FROM
            hop_dong AS hd
        JOIN khach_hang AS kh ON kh.id_khach_hang = hd.id_khach_hang
        
        WHERE
            hd.ngay_lam_hop_dong < '2016-01-01') AS c);
...
WHERE
    EXISTS( SELECT 
        ...
        FROM
            (SELECT 
               dvdk.ten_dich_vu_di_kem , SUM(hdct.so_luong)
            FROM
                dich_vu_di_kem as dvdk
            JOIN hop_dong_chi_tiet as hdct ON hdct.id_dich_vu_di_kem = dvdk.id_dich_vu_di_kem
            GROUP BY dvdk.id_dich_vu_di_kem
            HAVING SUM(hdct.so_luong) > 5) AS c );
 -- task 20 
SELECT 
    khach_hang.id_khach_hang,
    khach_hang.ho_ten,
